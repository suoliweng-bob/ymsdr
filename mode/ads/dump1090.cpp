...
void modesAds(void)
{
    if (!sdr_create_directory(sdr_upload.storagepath))
    {
        sdr_log_error_core(SDR_LOG_NOTICE, 0, " 【worker进程】创建目录失败%s.....!", "./ads");
        exit(1);
    }
...
// #endif
    ...
    if (Modes.interactive) {signal(SIGWINCH, (sighandler_t)sigWinchCallback);}
#endif
    ...
    //Modes.filename = "/home/topeet/Linux/sdr/modes1.bin";
    //if (Modes.net_only) {
...
//     }
  //  if (Modes.net) modesInitNet();
    ...
    while (Modes.exit == 0) {
       ...
       // fprintf(stdout, "Modes.iDataOut = %d Modes.iDataOut = %d  Modes.iDataLost = %d  \n", Modes.iDataOut, Modes.iDataOut, Modes.iDataLost);
