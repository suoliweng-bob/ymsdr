...
void SpectrumVisualProcessor::process() {
    ...
    if (data && !data->empty()) {
        ...
        //** std::cout << " distribute::run  xxxxxx is_view:" << is_view << "  iqData->:" <<   iqData->frequency << "  execute:" <<   iqData->sampleRate <<" size:" << data->size()  << std::endl;
        if (is_view) {
            ...
            if (centerFreq != iqData->frequency) {
                if ((centerFreq - iqData->frequency) != shiftFrequency || lastInputBandwidth != iqData->sampleRate) {
                    if (abs(iqData->frequency - centerFreq) <  (CbSDR::GetInstance()->getSampleRate() / 2) ) {
...
//                                        memset(&fft_result_peak[0], 0, numShift * sizeof(double));
        ...
        //** std::cout << " distribute::run  xxxxxxyy: num_written:" << num_written << "  fftSizeInternal:" <<   fftSizeInternal << "  execute:" <<   execute << std::endl;
        if (execute) {
           ...
           /*   std::cout << " distribute::run  xxxxxx: out.fft_ceiling:" << output->fft_ceiling << "  fft_floor:" <<   output->fft_floor <<" centerFreq:"<<output->centerFreq  << " bandwidth:"<<output->bandwidth<< std::endl;
            std::cout <<  "spectrum_points:";
            for(auto &p:output->spectrum_points)
            {
                     std::cout << p << "-";
            }
            std::cout << std::endl;
            std::cout << "  szie:" <<  output->spectrum_points.size()<< std::endl;
            exit(0);  */
            distribute(output);
        }
    }  
