...
void FFTDataDistributor::process() {
    //**std::cout << " FFTDataDistributor::process() xxxxxx" << std::endl;
	while (!input->empty()) {
		if (!isAnyOutputEmpty()) {
			return;
		}
        ...
        if (!input->pop(inp, HEARTBEAT_CHECK_PERIOD_MICROS)) {
         ...
         std::cout << " FFTDataDistributor::process() input::>pop date[].real:" << inp->data[i].real << " imag:" <<   inp->data[i].imag <<" size:"<<inp->data.size()<< std::endl;
         exit(0);  */
		if (inp) {
            //**std::cout << " FFTDataDistributor::process() input::>pop inputBuffer.sampleRate:" << inputBuffer.sampleRate << "  inp->sampleRate:" <<   inp->sampleRate <<" inputBuffer.frequency"<<inputBuffer.frequency  << " inp->frequency"<<inp->frequency<< std::endl;
