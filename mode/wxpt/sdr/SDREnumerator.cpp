...
std::vector<SDRDeviceInfo *> *SDREnumerator::enumerate_devices(const std::string& remoteAddr, bool noInit) {
    ...
    if (!soapy_initialized) {
        ...
        //std::string userModPath = nullptr/* wxGetApp().getModulePath() */;
        
        if (true/* !userModPath.empty() */) {
            /* wxGetApp().sdrEnumThreadNotify(SDREnumerator::SDR_ENUM_MESSAGE, "Loading SoapySDR modules from " + userModPath + ".."); */
            std::cout << "\tuserModPath.empty()... " << std::endl << std::flush;
            ...
            if (localMods.empty()) std::cout << "No modules found!" << std::endl << std::flush;
        } else {
        ...
        if ((factoryList.size() == 1) && factoryList.find("null") != factoryList.end()) {
           ...
           CbSDR::GetInstance()->sdrEnumThreadNotify(SDREnumerator::SDR_ENUM_FAILED, std::string("No modules available."));
        }
        std::cout << std::endl;
    ...
    for (size_t i = 0; i < results.size(); i++) {
        ...
        if (i<manualsIdx || manualResult[i-manualsIdx]) try {
           ...
           /*  if (isRemote) {
            ...
            DeviceConfig *cfg = CbSDR::GetInstance()->getConfig()->getDevice(dev->getDeviceId()) ;
...
void SDREnumerator::run() {
   ...
   CbSDR::GetInstance()->sdrEnumThreadNotify(SDREnumerator::SDR_ENUM_DEVICES_READY, "Finished scanning devices."); 
