...
//    SoapySDR_setLogLevel(SOAPY_SDR_DEBUG);
    ...
    SDRDeviceInfo *devInfo = deviceInfo.load();
     deviceConfig.store(CbSDR::GetInstance()->getConfig()->getDevice(devInfo->getDeviceId())); 
    ...
    if (!stream) {
        CbSDR::GetInstance()->sdrThreadNotify(SDRThread::SDR_THREAD_FAILED, std::string("Stream setup failed, stream is null. ") + streamExceptionStr); 
    ...
    if (device->hasDCOffsetMode(SOAPY_SDR_RX, 0)) {
        hasHardwareDC.store(true);
            CbSDR::GetInstance()->sdrEnumThreadNotify(SDREnumerator::SDR_ENUM_MESSAGE, std::string("Found hardware DC offset correction support, internal disabled."));
   ...
   CbSDR::GetInstance()->sdrThreadNotify(SDRThread::SDR_THREAD_INITIALIZED, std::string("Device Initialized.")); 
...
int SDRThread::readStream(const SDRThreadIQDataQueuePtr& iqDataOutQueue) {
    ...
    //be reached and the CbSDR displays "slows down". 
    ...
    //**std::cout << " numOverflow:" << numOverflow << " readStreamCode:" << readStreamCode << " stopping:" << stopping <<  " nElems:" << nElems <<   " n_read:" << n_read << " xxxxxx"<< std::endl;
    ...
    while (n_read < nElems && !stopping) {
        ...
        //**std::cout << "n_stream_read:" << n_stream_read << " nElems:" << nElems << "stopping:" << stopping <<  " xxxxxx"<< std::endl;
    ...
    if (n_read > 0 && !stopping && !iqDataOutQueue->full()) {
        ...
        if (collection_status.load() && !iqDataOutQueue->try_push(dataOut)) {
...
void SDRThread::updateSettings() {
    ...
    if (doUpdate) {
        ...
        CbSDR::GetInstance()->sdrThreadNotify(SDRThread::SDR_THREAD_INITIALIZED, std::string("Settings updated.")); 
    }
}
...
void SDRThread::setDevice(SDRDeviceInfo *dev) {
    ...
    if (dev) {
        ...
        deviceConfig.store(CbSDR::GetInstance()-> getConfig()->getDevice(dev->getDeviceId())); 
    } else {
        deviceConfig.store(nullptr);
    }
}
...
void SDRThread::setCollectionStatus(bool status) {
    collection_status.store(status);
}

bool SDRThread::getCollectionStatus() {
    return collection_status.load();
}
