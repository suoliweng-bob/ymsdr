...
void SDRPostThread::updateActiveDemodulators() {
    ...
    long long centerFreq = CbSDR::GetInstance()->getFrequency()/* wxGetApp().getFrequency() */;

    //retrieve the current list of demodulators:
    /* auto demodulators = wxGetApp().getDemodMgr().getDemodulators(); */
     auto demodulators = CbSDR::GetInstance()->getDemodMgr().getDemodulators(); 
    ...
    for (const auto& demod : demodulators) {
        ...
        if (abs(frequency - demod->getFrequency()) > (sampleRate / 2)) {
            ...
            if (CbSDR::GetInstance()->getDemodMgr().getCurrentModem()  /* wxGetApp().getDemodMgr().getCurrentModem()  */ == demod) {
            ...
            if (demod->isFollow() && centerFreq != demod->getFrequency()) {
                ...
                CbSDR::GetInstance()->setFrequency(demod->getFrequency());
                demod->setFollow(false);
            }
        } else if (!demod->isActive()) { // in range, activate if not activated
            demod->setActive(true);
            if (CbSDR::GetInstance()->getDemodMgr().getCurrentModem()/* wxGetApp().getDemodMgr().getCurrentModem()  */ == nullptr) {
                /* wxGetApp().getDemodMgr().setActiveDemodulator(demod); */
                CbSDR::GetInstance()->getDemodMgr().setActiveDemodulator(demod);
            }
        }
...
void SDRPostThread::run() {
...
#else
    iqActiveDemodVisualQueue = nullptr;
#endif
    //**std::cout << "SDR post-processing thread started.xxxxxxxxxxxxxxxxxxxxxxxxxxxxxx  stop;" << stopping << std::endl;
    while (!stopping) {
      ...
      //** std::cout << "channel :" <<  data_in->numChannels <<  "  chanMode:" << chanMode << std::endl;
       //**std::cout << "data_in->data. : " <<  data_in->data.data()  << std::endl;
        ...
        //**std::cout << "  doUpdate:" << doUpdate  << "  doRefresh.load():" << doRefresh.load() << std::endl;
    ...
    if (iqActiveDemodVisualQueue != nullptr)
...
void SDRPostThread::runDemodChannels(int channelBandwidth) {
    DemodulatorInstancePtr activeDemod = CbSDR::GetInstance()->getDemodMgr().getCurrentModem()/* wxGetApp().getDemodMgr().getCurrentModem() */;
