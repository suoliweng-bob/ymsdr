...
std::string AppConfig::getConfigDir() {
     ...
     std::string ycDir = std::string(".") + std::string(g_app_name);
    std::string dataDir  = std::string("./") +  ycDir;

    if (envVar)
    {
        dataDir = std::string(envVar) + std::string("/") + ycDir;
        sdr_create_directory(dataDir.c_str());
    }
...
bool AppConfig::verifyRecordingPath() {
    // string recPathStr = CbSDR::GetInstance()->getConfig()->getRecordingPath();
    ...
    if (recordingPath.empty()) {
        ...
        recordingPath = std::string(sdr_upload.storagepath);
    }
    
    std::cout << " RecordingPath:" << recordingPath << std::endl;
    return sdr_create_directory(recordingPath.c_str());
}
...
std::string AppConfig::getConfigFileName(bool ignoreName) {
     ...
     */
    std::string cfgFileName = cfgFileDir + std::string("/config.xml") ;/* cfgFile.GetFullPath(wxPATH_NATIVE).ToStdString(); */
    return cfgFileName;
}
...
bool AppConfig::save() {
    ...
    cfg.rootNode()->setName("sdr_config");
...
bool AppConfig::load() {
    ...
    std::string cfgFileName = getConfigFileName();
    std::cout << "AppConfig::load() cfgFileName:" << cfgFileName  << std::endl;
   ...
   /*  wxFileName cfgFile = wxFileName(cfgFileName);
    ...
    if (sdr_file_exists(cfgFileName.c_str())/* cfgFile.IsFileReadable() */) {
