...
void DemodulatorInstance::setSquelchLevel(float signal_level_in) {
    ...
    CbSDR::GetInstance()->getDemodMgr().setLastSquelchLevel(signal_level_in);
    CbSDR::GetInstance()->getDemodMgr().setLastSquelchEnabled(true); 
}
...
void DemodulatorInstance::setMuted(bool muted_in) {
    ...
    CbSDR::GetInstance()->getDemodMgr().setLastMuted(muted_in);
}
...
void DemodulatorInstance::startRecording() {
    ...
    std::string userLabelStr = getLabel(); 
    std::string  userType = CbSDR::GetInstance()->getAudioType();
    if (!userLabelStr.empty() && !userType.empty()) {
        fileName << userLabelStr + "_" + userType;
    } else {
        fileName << "rk";
    }
	...
	newSinkThread->setFileTimeLimit(wxGetApp().getConfig()->getRecordingFileTimeLimit()); */
      ...
      std::cout << "getRecordingSquelchOption:" <<  CbSDR::GetInstance()->getConfig()->getRecordingSquelchOption()<< std::endl;
       std::cout << "getRecordingFileTimeLimit:" <<  CbSDR::GetInstance()->getConfig()->getRecordingFileTimeLimit()<< std::endl;
       std::cout << "getRecordingPath:" <<  CbSDR::GetInstance()->getConfig()->getRecordingPath()<< std::endl;
       
     newSinkThread->setSquelchOption(CbSDR::GetInstance()->getConfig()->getRecordingSquelchOption());
