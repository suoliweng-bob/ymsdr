...
//    std::cout << "Demodulator worker thread started.." << std::endl;
    ...
    while (!stopping) {
        ...
        if ((makeDemod || filterChanged) && !stopping) {
            ...
            result.modem = cModem;
            std::cout << "cModemName:"<< cModemName  << " cModemType:" << cModemType  << " demodCommand.sampleRate:" << demodCommand.sampleRate << std::endl;
            if (makeDemod && demodCommand.bandwidth && demodCommand.audioSampleRate) {
                if (cModem != nullptr) {
                    ...
                    std::cout << "demodCommand.bandwidth,:"<< demodCommand.bandwidth  << " demodCommand.audioSampleRate:" << demodCommand.audioSampleRate  << std::endl;
                } else {
                    cModemKit = nullptr;
                }
                ...
                if (cModem != nullptr) {
                    ...
                    std::cout << "filterCommand.bandwidth,:"<< filterCommand.bandwidth  << " filterCommand.audioSampleRate:" << filterCommand.audioSampleRate  << std::endl;
                } else {
                    cModemKit = nullptr;
                }
            } else if (makeDemod) {
