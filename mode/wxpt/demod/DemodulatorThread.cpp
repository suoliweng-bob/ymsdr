...
//    std::cout << "Demodulator thread started.." << std::endl;
    ...
    while (!stopping) {
        ...
        if (!iqInputQueue->pop(inp, HEARTBEAT_CHECK_PERIOD_MICROS)) {
         ...
         //**std::cout <<  "DemodulatorThread::run iqInputQueue->pop( xxxxxxx"<< std::endl;
        ...
        //std::cout << currentSignalLevel << ": "<<  inp->data.size()<< ": " << inp->sampleRate<< ": " << sampleTime<< ": "  << signalFloor << ": "<<  signalCeil << ": "<<  squelchLevel << ": "<< std::endl;
        ...
        if (squelchEnabled) {
            if (!squelched && !squelchBreak) {
                    if (CbSDR::GetInstance()->getSoloMode() /* wxGetApp().getSoloMode() /*&& !wxGetApp().getAppFrame()->isUserDemodBusy() */) {
                        std::lock_guard < std::mutex > lock(squelchLockMutex);
                        if (squelchLock == nullptr) {
                            std::cout << "Demodulator setActiveDemodulator..xxxxxxxx" << std::endl;
                           ...
                           CbSDR::GetInstance()->getDemodMgr().setActiveDemodulator(nullptr);
                            CbSDR::GetInstance()->getDemodMgr().setActiveDemodulatorByRawPointer(demodInstance, false); 
                            squelchBreak = true;
       ...
       /*  if (!squelched && ati != nullptr) {
            if (!muted &&(!CbSDR::GetInstance()->getSoloMode()|| (demodInstance ==
                    CbSDR::GetInstance()->getDemodMgr().getCurrentModem().get())) ) {
        ...
        if (ati && localAudioSinkOutputQueue != nullptr) {
            //std::cout << " DemodulatorThread::run()  push ati into audioSinkOutputQueue "<< std::endl;
