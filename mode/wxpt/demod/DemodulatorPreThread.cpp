...
//    std::cout << "Demodulator preprocessor thread started.." << std::endl;
    ...
    while (!stopping) {
        ...
        if (demodTypeChanged.load() && (newSampleRate && newAudioSampleRate && currentBandwidth)) {
            //**std::cout << "DemodulatorPreThread::run() DEMOD_WORKER_THREAD_CMD_MAKE_DEMODxxxxxx  inp->sampleRate:" << inp->sampleRate   << " currentSampleRate:" << currentSampleRate<< "  cModem:"<< cModem <<  "  cModemKit:" << cModemKit<< std::endl;
            ...
            //**std::cout << "DemodulatorPreThread::run() DEMOD_WORKER_THREAD_CMD_BUILD_FILTERS  inp->sampleRate:" << inp->sampleRate   << " currentSampleRate:" << currentSampleRate<< "  cModem:"<< cModem <<  "  cModemKit:" << cModemKit<< std::endl;
            command.frequency = newFrequency;
            command.sampleRate = newSampleRate;
            command.bandwidth = currentBandwidth;
            command.audioSampleRate = newAudioSampleRate;
...
//        std::lock_guard < std::mutex > lock(inp->m_mutex);
        ...
        //**std::cout << "DemodulatorPreThread::run() xxxxxx  inp->sampleRate:" << inp->sampleRate   << " currentSampleRate:" << currentSampleRate<< "  cModem:"<< cModem <<  "  cModemKit:" << cModemKit<< std::endl;
        //exit(10);
