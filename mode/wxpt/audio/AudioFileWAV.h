...
class SpectrumFileCSV : public SpectrumFile {

public:
    SpectrumFileCSV();
    ~SpectrumFileCSV() override;

	//override to manage name change with multi-part CSV. 
	void setOutputFileName(std::string filename) override;
    ...
    bool writeToFile(SpectrumVisualDataPtr input) ;
    bool closeFile() override;
...
private:
	...
	size_t getMaxWritableNumberOfSamples(const SpectrumVisualDataPtr& input) const;

	void writeHeaderToFileStream(const SpectrumVisualDataPtr& input);

	//write [startInputPosition; endInputPosition[ samples from input into the file.
	void writePayloadToFileStream(const SpectrumVisualDataPtr& input, size_t inputSize);
};