...
class SpectrumSinkFileThread : public SpectrumSinkThread {

public:
    SpectrumSinkFileThread();
    ~SpectrumSinkFileThread() override;


    void sink(SpectrumVisualDataPtr input) override;
    void inputChanged(SpectrumVisualData oldProps, SpectrumVisualDataPtr newProps) override;

    void setSectrumFileHandler(SpectrumFile *output);

	void setSectrumFileNameBase(const std::string& baseName);
...
protected:
    ...
    SpectrumFile *audioFileHandler = nullptr;
