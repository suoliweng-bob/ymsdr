...
public:
   ...
   void toggleActiveDemodRecording();
    ...
    void setSdrCollectionStatus(bool status);
    bool getSdrCollectionStatus();
...
private:
  ...
  /*   PrimaryGLContext *m_glContext = nullptr;
    ...
    std::atomic_bool freqChange;
     std::atomic_bool typeChange;
     std::atomic_bool collectionStatus;
    ...
    //std::shared_ptr<CUBServer> cubServer = std::make_shared<CUBServer>();
    CUBServer * cubServerThread = nullptr;
    SpectrumSinkThread *spectrumSinkThread = nullptr;
    std::thread *t_SpectrumSink = nullptr;
    ...
    CliDataQueuePtr pipeCliQueueData = std::make_shared<CliDataQueue>();
    ...
    std::thread *t_DemodVisual = nullptr;
     std::thread *t_CliServer = nullptr;
...
private:
	static CbSDR *m_instance;
	
public:
	static CbSDR* GetInstance()
	{
		if(m_instance == NULL)
		{
			if(m_instance == NULL)
			{
				m_instance = new CbSDR();
				static Cubichuishou cl;
			}
		}
		return m_instance;
	}
	class Cubichuishou	// 用于释放对象
	{
	public:
		~Cubichuishou()
		{
			if(CbSDR::m_instance)
			{
				delete CbSDR::m_instance;
				CbSDR::m_instance = NULL;
			}
		}
