...
void BookmarkMgr::saveToFile(const std::string& bookmarkFn, bool backup, bool useFullpath) {
    ...
    header->newChild("version")->element()->set(std::string("v1.0"));//**wxString(CUBICSDR_VERSION).ToStdWstring());

    DataNode *branches = s.rootNode()->newChild("branches");
    
    *branches->newChild("active") = 1;//**wxGetApp().getAppFrame()->getBookmarkView()->getExpandState("active")?1:0;
    *branches->newChild("range") = 1;//**wxGetApp().getAppFrame()->getBookmarkView()->getExpandState("range")?1:0;
    *branches->newChild("bookmark") =1;//** wxGetApp().getAppFrame()->getBookmarkView()->getExpandState("bookmark")?1:0;
    *branches->newChild("recent") = 1;//**wxGetApp().getAppFrame()->getBookmarkView()->getExpandState("recent")?1:0;
    ...
    for (auto &bmd_i : bmData) {
        ...
        *group->newChild("@expanded") = (getExpandState(bmd_i.first)?std::string("true"):std::string("false"));
        ...
        for (auto &bm_i : bmd_i.second ) {
			...
			auto matchingDemod = CbSDR::GetInstance()->getDemodMgr().getLastDemodulatorWith(bm_i->type,
				bm_i->label,
				bm_i->frequency,
				bm_i->bandwidth);
			...
			if (matchingDemod != nullptr) {
				...
				CbSDR::GetInstance()->getDemodMgr().saveInstance(group->newChild("modem"), matchingDemod);
			}
			else {
    ...
    for (const auto& demod : CbSDR::GetInstance()->getDemodMgr().getDemodulators()) {
        CbSDR::GetInstance()->getDemodMgr().saveInstance(recent_modems->newChild("modem"),demod);
    }
	...
	std::string saveFile;
	std::string saveFileBackup;

	if (useFullpath) {
		saveFile = bookmarkFn;
		saveFileBackup = bookmarkFn + std::string(".backup");
	}
	else {
		saveFile = CbSDR::GetInstance()->getConfig()->getConfigDir() +std::string("/") + bookmarkFn;
		saveFileBackup = CbSDR::GetInstance()->getConfig()->getConfigDir() + std::string("/") + bookmarkFn + std::string(".backup");
	}
    std::cout << "SaveToFileXML(saveFile)" << saveFile << std::endl;
    s.SaveToFileXML(saveFile);
    /* if (sdr_file_exists(saveFile.c_str())) {
...
bool BookmarkMgr::loadFromFile(const std::string& bookmarkFn, bool backup, bool useFullpath) {
	...
	if (useFullpath) {
		loadFile = bookmarkFn;
	/* 	failFile = bookmarkFn + std::string(".failedload");
		lastLoaded = bookmarkFn + std::string(".lastloaded");
		backupFile = bookmarkFn + std::string(".backup");	 */
	}
	else {
		loadFile = CbSDR::GetInstance()->getConfig()->getConfigDir() + std::string("/") + bookmarkFn;
	/* 	failFile = wxGetApp().getConfig()->getConfigDir() + bookmarkFn + std::string(".failedload");
		lastLoaded = wxGetApp().getConfig()->getConfigDir() + bookmarkFn + std::string(".lastloaded");
		backupFile = wxGetApp().getConfig()->getConfigDir() + bookmarkFn + std::string(".backup"); */
	}
    std::cout << " loadFile:" << loadFile << std::endl;
    DataTree s;
    ...
    if (backup && !sdr_file_exists(loadFile.c_str())/* && !lastLoaded.FileExists() && !backupFile.FileExists() */) {
        loadDefaultRanges();
        return true;
    }
    
    // Attempt to load file
    if (!s.LoadFromFileXML(loadFile)) {
        return false;
    }
...
void BookmarkMgr::updateActiveList() {
    ...
    if (CbSDR::GetInstance()->isShuttingDown()) {
        return;
    }
...
BookmarkEntryPtr BookmarkMgr::demodToBookmarkEntry(const DemodulatorInstancePtr& demod) {
    ...
    CbSDR::GetInstance()->getDemodMgr().saveInstance(be->node, demod);
...
void BookmarkMgr::removeActive(const DemodulatorInstancePtr& demod) {
	...
	//Delete demodulator
	CbSDR::GetInstance()->getDemodMgr().setActiveDemodulator(nullptr, true);
	CbSDR::GetInstance()->getDemodMgr().setActiveDemodulator(nullptr, false);
	CbSDR::GetInstance()->removeDemodulator(demod);
	CbSDR::GetInstance()->getDemodMgr().deleteThread(demod);
}
